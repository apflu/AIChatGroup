...
`io/gateway/` 是多套适配器 + 一个路由器。规范线格式是「Anthropic 形状」（system block +
...
`RouterGateway` 只看分隔符 `::` 前的别名分发（换 provider 只改别名、不动模型名；同一模型名可挂在
不同端点上互不影响），自身也实现 `ModelGateway` 协议 → engine / Orchestrator **完全不用改**。
不带分隔符的裸模型名仍按前缀推断路由（`#` 仍作旧分隔符兼容）。于是角色卡里给不同角色写不同 `model_id` 即可混用多家。
...
气泡之间的停顿由 [pacing.py](src/aichatgroup/message/delivery/pacing.py) 推断，结果放在 `TurnResult.pauses`，
