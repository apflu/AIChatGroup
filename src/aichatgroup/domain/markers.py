...
# 动作/语言分离：举动写 `{{ACT:…}}`（冒号式；旧 `{{ACTION}}…{{/ACTION}}` 仍被解析器容忍），
# 神态用 RP 惯用的 `*…*`；其余为语言。解析归到 ContentPart(kind=beat|gesture|speech)，
# 正则在 message/generator/parsing.py，教法在 prompts/role/output_contract.md。

# 回复寻址：模型在一条气泡开头写 `{{REPLY:37}}` 表示回复历史里 ⟦37⟧ 那条。
# 引擎剥掉标记、把内部 id 存进该气泡的 reply_to（见 engine/parsing.py）。
REPLY_MARKER = "{{REPLY}}"

# 发言者种类标签：渲染进历史时给**人类玩家**的行加，标出"这是人、不是 AI 角色"。
# 只**读**不产（AI 角色的行永远不带它 → 模型没样可学、几乎不会回显），故刻意用 `<...>` 的
# markup 观感——要的正是它的**刺目/难伪造**：与自然中文名字撕得开、玩家名字里又被净化禁掉
# `<>`（见 domain/player.py::sanitize_player_name），无从伪造。位置在名字**前**，绝不做名字后缀。
# 这跟上面"避免 `<<...>>`"不矛盾：那条针对模型会**自动闭合**的**输出**标记；本 tag 只进不出。
USER_TAG = "<user>"
